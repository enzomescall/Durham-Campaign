...
baker_v_caballero_hth = head_to_head(baker_v_caballero, c("Nate Baker", "Javiera Caballero", "Carl Rist", "Khalilah Karim", "Monique Holsey-Hyman", "Shelia Ann Huggins"))
johnson_v_reece_hth = head_to_head(johnson_v_reece, c("Jillian Johnson", "Charlie Reece", "Javiera Caballero", "Daniel Meier", "Jacqueline Wagstaff"))
...
full = hth_df %>%
  ...
  left_join(candidates_df %>% select(-id, year), by = c("c2_name" = "name"), suffix = c(".c1", ".c2")) %>%
...
model = lmer(result ~ (1|Precinct) + PA.c1 + Indy.c1 + Committee.c1 + PA.c2 + Indy.c2 + Committee.c2, data = full)
```
```{r}
summary(model)
```
